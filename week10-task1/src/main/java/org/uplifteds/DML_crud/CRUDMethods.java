...
public class CRUDMethods {
    ...
    private static void findListOfStudentsInResultSet(ResultSet resultSet) throws SQLException {
        ...
        while (resultSet.next()) {
            User stud = new User();
            stud.setId(resultSet.getInt(User.idFieldName));
            stud.setName(resultSet.getString(User.nameFieldName));
            stud.setSurname(resultSet.getString(User.surnameFieldName));
            stud.setBirthdate(resultSet.getDate(User.birthdateFieldName));
            userList.add(stud);
        }
        resultSet.close();
        if (userList.size() > 0) {
            for (User tempStud : userList) {
